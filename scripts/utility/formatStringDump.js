...
const relativePath = '../../textFilesToBeConverted/launchTitles/xavixPortLaunchTitles.json';
const idPrefix = 'sxplt';

(async function() {
  const contents = await fileUtil.readFile(relativePath);
  const parsed = JSON.parse(contents);
  const newData = fileUtil.stringArrToObjectArr(parsed, 'XaviXPORT launch titles', idPrefix);
