...
const relativePath = '../../textFilesToBeConverted/launchTitles/wonderswanLaunchTitles.json';
const idPrefix = 'wslt';

(async function() {
  const contents = await fileUtil.readFile(relativePath);
  const parsed = JSON.parse(contents);
  const newData = fileUtil.stringArrToObjectArr(parsed, 'Bandai WonderSwan launch title', idPrefix);
