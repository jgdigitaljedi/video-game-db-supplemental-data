...
const relativePath = '../../textFilesToBeConverted/platformExclusives/atariJaguarCDExclusives.json';
const idPrefix = 'ajcdex';

(async function() {
  const contents = await fileUtil.readFile(relativePath);
  const parsed = JSON.parse(contents);
  const newData = fileUtil.stringArrToObjectArr(parsed, 'Atari Jaguar CD exclusive game', idPrefix);
